...
In essence, entropy is a measure of the amount of uncertainty in a system. A system with more entropy can reveal more information. Mathematically, it's measured by the Shannon formula:

$H(X) = −∑ p(x) * log p(x)$

![Binary Entropy Plot](/media/gambling-entropy.png)

As we described above, the entropy is the highest when the probability is 50-50.

## Usefulness of Information Theory

Information Theory is relevant to a lot of fields, but I will explain two big ones - Machine Learning and Cryptography.
...
However, if we have some training data, then we can instead consider the conditional entropy of the prediction outcome `X` given the data we have already observed. A machine learning algorithm that works well would aim to minimise this entropy in `X` using more relevant training data and methods.
...
Encoding and decoding bits is at the core of information theory. Given a message like "hello, welcome to this site",

How many bits do we need to encode it such that
...
![Text Compression and Lossiness](/media/gambling-compression.png)
...
**Language Compression: Stenography**

An interesting use case of language compression is in **stenography**, where a few buttons can be used to effectively communicate the English language. One action to press multiple keys on the keyboard can generate a combination of letters, which gets programmatically translated to words. 

`KP A PL \ PL`, requiring two hand actions, is sufficient to type the word "example". Traditional QWERTY keyboards require 7 key presses to achieve the same result.

![Stenography Example](/media/stenography.png)

Stenography is used in legal or parliamentary sessions, where there is a need to quickly encode what was verbally discussed into text. 
...
For my term project in the module, I did a report on Gambling and Data Compression, from Cover and Thomas' chapter (Elements of Information Theory, Chapter 6). The authors were using horse races for English text prediction.
...
![Gambling and Text Prediction](/media/gambling-text-equivalence.png)

The predictor is basically a horse bettor, making bets on the next letter that will occur in a given sequence of letters. Hence, if the gambler can make good predictions and earn a lot of wealth, the predictor can also make good predictions for the English language.

The report is targeted at readers familiar with basic notations in Information Theory. If you'd like to learn more about the topic, feel free to leave your thoughts in the comments. Interested readers can read the full analysis written by me [here](/docs/gambling.pdf).

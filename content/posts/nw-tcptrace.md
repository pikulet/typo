...
Usually, traceroute is implemented over UDP. Packets are sent with an increasing time-to-live (TTL) value in the IP header. When the TTL (Time-to-Live) expires, an ICMP (Internet Control Message Protocol) error message is sent back to the host. Information on the intermediate hops is extracted from these ICMP error messages.
...
![TTL-based Probing](/media/tcp-traceroute-ttl.svg)

### Why implement traceroute using TCP?

Some hosts have firewalls that disable UDP messages, as most applications running over HTTP also use TCP. Firewalls are more permissive toward TCP because most web services are built over TCP.

## TCP traceroute mechanism

The TCP traceroute mechanism is also error-based. The expiring TTL mechanism is used to detect the intermediate hops.

![UDP vs TCP Traceroute](/media/tcp-traceroute-comparison.svg)

 Interestingly, the probing packet is a TCP SYN packet, which is used to start a TCP 3-way handshake. The packet is also sent to an unused port which is not interested in starting a 3-way handshake. Two scenarios can arise, and the port can return an ICMP port unreachable error. However, it is more common to encounter the TCP RST packet. These RST packets are used when an unexpected TCP packet arrives at a host.
...
Using raw sockets and pseudoheaders is really complicated. That's when my classmate proposed using TCP sockets (`SOCK_STREAM`). The `connect()` routine initiates a three-way handshake, which triggers a TCP SYN packet to be sent!
...
![TCP Traceroute Example Output](/media/tcptrace-example.png)

...
Suppose we have four machines in our kubernetes cluster available to run our tasks. We need to decide between two kinds of scheduling strategies, (a) compact and (b) balanced, which affects the load balancing between the different machines.
...
By default, kubernetes comes with its own `kube-scheduler`, though we can define our own scheduler service to overwrite it. This article explores how we can achieve balanced scheduling using a **descheduler**.

## Resource Specification: Requests vs Limits

In kubernetes, resources are defined with two parameters - requests and limits. Every resource type will have both a request and limit, and this can be extended from CPU to memory, GPU and even sockets. These request and limit values are defined by the users themselves, and provide the kube-scheduler with an indication of how much resources the task requires.

...
Suppose that your application is currently using a database, but it encounters increasing write pressure, e.g. due to an increasing user base. At this time, the database administrator might recommend **sharding** the database (deploying across more machines).
...
More often than not, applications would prefer to carry out **online migration**, where the database has no downtime.

### Types of Database Migration

![Overview](/media/db-migration-overview.png)

First, I'll give an overview of how database migrations are commonly performed. There are two main types of migration, namely scan-based and binlog-based.

- **Scan-based** migration: read from the old database (`SELECT`), and then write to the new database.

- **Binlog-based** replication: read an event from the binary log (binlog), and then replicate that event to the new database. The binlog is a sequential log of **data changes** (e.g. add 1 to the score of ID=2), and doesn't contain the full row data.

Binlog replication is used in master-slave replication. Each time the application writes to the database master, the slave reads the change and copies it. Reading the change is more efficient than scanning the entire database to copy an entire row.
...
![Scan and Write Order](/media/db-migration-order.png)
...
However, we also need to consider that writes can occur before scans. The application deletes the row `id=30000` before the scan process has copied `id=30000` over. If we follow the chronological order of events, there's a need to **wait** for the scan process before the write. Otherwise, the migration process first deletes the row in the new database, before the scan process incorrectly inserts the row again.

![Write Before Scan: Ghost Row Problem](/media/db-migration-write-before-scan.svg)

However, waiting for the scan to complete is not viable. The binlog could expire during the process of waiting for the scan to complete. 
...
Different systems have their own techniques to cover these pitfalls of database migration, one of which being the better-known [gh-ost](https://github.com/github/gh-ost) by GitHub.
